...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "seremis.db")
...
def init_db():
    with db_conn() as conn:
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_notif_user        ON notificaciones(userId, leida);
        """)
        conn.commit()

        _seed_seremis(conn)
        _seed_users(conn)
        _seed_proyectos(conn)
        _seed_nudos(conn)
        _seed_temas(conn)
        _seed_agenda(conn)
        _seed_prensa(conn)
        _seed_visitas(conn)
        _seed_contrataciones(conn)
        conn.commit()
...
def _seed_seremis(conn):
    if _count(conn, "seremis") > 0:
        return
    rows = [
    ...
    conn.executemany(
        "INSERT INTO `seremis` (`id`,`sector`,`nombre`,`c1`,`c2`) VALUES (?,?,?,?,?)", rows
    )
...
def _seed_users(conn):
    ...
    rows = [
        ("admin", "admin", pwd_ctx.hash("admin123"), "admin", None, "Administrador Regional", "Admin", "admin@gore-maule.cl", ""),
        ("salud", "salud", pwd_ctx.hash("seremi123"), "seremi", "salud", "Carmen López", "Dra.", "clopez@minsal.cl", "+56 9 1111 0001"),
        ("educacion", "educacion", pwd_ctx.hash("seremi123"), "seremi", "educacion", "Rodrigo Valdivia", "Sr.", "rvaldivia@mineduc.cl", "+56 9 1111 0002"),
        ("obras", "obras", pwd_ctx.hash("seremi123"), "seremi", "obras", "Felipe Meza", "Ing.", "fmeza@mop.gov.cl", "+56 9 1111 0003"),
        ("agricultura", "agricultura", pwd_ctx.hash("seremi123"), "seremi", "agricultura", "Marcela Torres", "Ing.", "mtorres@minagri.gob.cl", "+56 9 1111 0004"),
        ("vivienda", "vivienda", pwd_ctx.hash("seremi123"), "seremi", "vivienda", "Sandra Rojas", "Arq.", "srojas@minvu.cl", "+56 9 1111 0005"),
        ("transporte", "transporte", pwd_ctx.hash("seremi123"), "seremi", "transporte", "Pablo Ibáñez", "Sr.", "pibanez@mtt.gob.cl", "+56 9 1111 0006"),
        ("bienes", "bienes", pwd_ctx.hash("seremi123"), "seremi", "bienes", "Antonio Fuentes", "Sr.", "afuentes@bienes.cl", "+56 9 1111 0007"),
        ("trabajo", "trabajo", pwd_ctx.hash("seremi123"), "seremi", "trabajo", "Claudia Vera", "Sra.", "cvera@mintrab.gob.cl", "+56 9 1111 0008"),
        ("medioambiente", "medioambiente", pwd_ctx.hash("seremi123"), "seremi", "medioambiente", "Valentina Soto", "Dra.", "vsoto@mma.gob.cl", "+56 9 1111 0009"),
        ("energia", "energia", pwd_ctx.hash("seremi123"), "seremi", "energia", "Cristóbal Ramos", "Ing.", "cramos@energia.gob.cl", "+56 9 1111 0010"),
        ("economia", "economia", pwd_ctx.hash("seremi123"), "seremi", "economia", "Fernanda Lagos", "Mg.", "flagos@economia.cl", "+56 9 1111 0011"),
        ("mineria", "mineria", pwd_ctx.hash("seremi123"), "seremi", "mineria", "Jorge Espinoza", "Ing.", "jespinoza@minmineria.cl", "+56 9 1111 0012"),
        ("desarrollosocial", "desarrollosocial", pwd_ctx.hash("seremi123"), "seremi", "desarrollosocial", "Patricia Muñoz", "Sra.", "pmunoz@mdsf.gob.cl", "+56 9 1111 0013"),
        ("justicia", "justicia", pwd_ctx.hash("seremi123"), "seremi", "justicia", "Nelson Tapia", "Abg.", "ntapia@minjusticia.cl", "+56 9 1111 0014"),
        ("interior", "interior", pwd_ctx.hash("seremi123"), "seremi", "interior", "Mauricio Flores", "Sr.", "mflores@interior.gob.cl", "+56 9 1111 0015"),
        ("cultura", "cultura", pwd_ctx.hash("seremi123"), "seremi", "cultura", "Isabel Contreras", "Sra.", "icontreras@cultura.gob.cl", "+56 9 1111 0016"),
        ("ciencia", "ciencia", pwd_ctx.hash("seremi123"), "seremi", "ciencia", "Andrés Bravo", "Dr.", "abravo@minciencia.gob.cl", "+56 9 1111 0017"),
        ("deporte", "deporte", pwd_ctx.hash("seremi123"), "seremi", "deporte", "Lucas Herrera", "Prof.", "lherrera@ind.cl", "+56 9 1111 0018"),
        ("mujer", "mujer", pwd_ctx.hash("seremi123"), "seremi", "mujer", "Andrea Castro", "Sra.", "acastro@sernameg.gob.cl", "+56 9 1111 0019"),
    ]
    conn.executemany(
...
def _seed_proyectos(conn):
    if _count(conn, "proyectos") > 0:
        return
    rows = [
...
def _seed_nudos(conn):
    if _count(conn, "nudos") > 0:
        return
    rows = [
...
def _seed_temas(conn):
    if _count(conn, "temas") > 0:
        return
    rows = [
...
def _seed_agenda(conn):
    if _count(conn, "agenda") > 0:
        return
    rows = [
...
def _seed_prensa(conn):
    if _count(conn, "prensa") > 0:
        return
    rows = [
...
def _seed_visitas(conn):
    if _count(conn, "visitas") > 0:
        return
    rows = [
        # salud
...
def _seed_contrataciones(conn):
    if _count(conn, "contrataciones") > 0:
        return
    rows = [
        ("salud", "María José Contreras", "15.234.567-8", "Profesional Sectorial", "Grado 10", "Honorarios", "Nuevo", "2026-03-01", "2026-12-31", "1850000", "Presupuesto SEREMI", "Refuerzo equipo epidemiológico para campaña invierno regional.", "Pendiente", "", "", "salud", "2026-02-18"),
        ("educacion", "Carlos Fuentes Riquelme", "12.876.543-2", "Analista de Datos", "Grado 12", "Contrata", "Nuevo", "2026-03-15", "2026-12-31", "1450000", "Programa sectorial", "Apoyo al Sistema de Información Escolar regional (SIGE).", "Aprobada", "Administrador Regional", "2026-02-17", "educacion", "2026-02-10"),
        ("obras", "Sofía Espinoza Vera", "16.123.456-7", "Ingeniero Vial", "Grado 9", "Honorarios", "Nuevo", "2026-04-01", "2026-10-31", "2100000", "Transferencia GORE", "Supervisión obras puente Río Claro sector Talca.", "Aprobada", "Administrador Regional", "2026-02-20", "obras", "2026-02-14"),
        ("vivienda", "Lorena Tapia Muñoz", "14.987.654-3", "Arquitecta Revisora", "Grado 8", "Contrata", "Nuevo", "2026-03-01", "2027-02-28", "1720000", "Presupuesto SEREMI", "Revisión permisos y proyectos DS19 para sector rural.", "Pendiente", "", "", "vivienda", "2026-02-21"),
        ("medioambiente", "Rodrigo Castro Ibáñez", "17.345.678-4", "Inspector Ambiental", "Grado 11", "Honorarios", "Nuevo", "2026-03-15", "2026-09-15", "1350000", "Presupuesto SEREMI", "Fiscalización norma SO2 cuenca Río Maule.", "Pendiente", "", "", "medioambiente", "2026-02-22"),
        ("agricultura", "Camila Rojas Figueroa", "13.654.321-K", "Técnico Agrícola", "Nivel 2", "Honorarios", "Nuevo", "2026-03-01", "2026-06-30", "980000", "Programa sectorial", "Apoyo programa emergencia hídrica zona secana interior.", "Aprobada", "Administrador Regional", "2026-02-19", "agricultura", "2026-02-12"),
    ]
    conn.executemany(
